...
def test_get_one_post_not_exist(authorized_client, test_posts):
    res = authorized_client.get(f"/api/v1/posts/88888")
...
@pytest.mark.parametrize("title, content, published", [
    ...
    ("tallest skyscrapers", "woohoo", True),
])
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post(
        "/api/v1/posts/", json={"title": "arbitrary title", "content": "aasdfjasdf"})
...
def test_unauthorized_user_delete_Post(client, test_user, test_posts):
    res = client.delete(
        f"/api/v1/posts/{test_posts[0].id}")
    assert res.status_code == 401
...
def test_delete_other_user_post(authorized_client, test_user, test_posts):
    res = authorized_client.delete(
        f"/api/v1/posts/{test_posts[3].id}")
...
def test_update_post_non_exist(authorized_client, test_user, test_posts):
    ...
    res = authorized_client.put(
        f"/api/v1/posts/8000000", json=data)
